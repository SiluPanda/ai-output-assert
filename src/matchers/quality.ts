...
import { DEFAULT_HEDGING_PHRASES } from '../catalogs/hedging-phrases';

export function toNotBeTruncated(received: string): MatcherResult {
  const issues: string[] = [];
...
export function toNotBeHedged(
  received: string,
  phrases?: string[],
  threshold = 0.3,
): MatcherResult {
  const allPhrases = [...DEFAULT_HEDGING_PHRASES, ...(phrases ?? [])];
...
export function toBeCompleteJSON(received: string): MatcherResult {
  const trimmed = received.trim();
  try {
    JSON.parse(trimmed);
    return {
    ...
    return {
      ...
      message: () =>
        truncated
          ...
          : `Expected output to be complete JSON, but it is not valid JSON: ${(parseError as Error).message}`,
      details: { truncated, parseError: (parseError as Error).message },
    };
  }
}

export function toNotRepeat(
  received: string,
  ...
  const ngrams = extractNgrams(tokens, windowSize);
  const counts = new Map<string, number>();
  for (const ng of ngrams) {
    counts.set(ng, (counts.get(ng) ?? 0) + 1);
  }
  const repeated: Array<{ ngram: string; count: number }> = [];
