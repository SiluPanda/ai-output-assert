...
export function toStartWith(received: string, prefix: string): MatcherResult {
...
export function toEndWith(received: string, suffix: string): MatcherResult {
...
export function toBeFormattedAs(received: string, format: OutputFormat): MatcherResult {
  let pass = false;
  let reason = '';
  ...
  switch (format) {
    ...
    case 'markdown': {
      ...
      if (!pass) reason = 'No markdown patterns detected (headings, bold, code, lists, blockquotes)';
      break;
    }
    case 'list': {
      ...
      if (!pass) reason = `Only ${listLines.length}/${lines.length} lines are list items`;
      break;
    }
    case 'csv': {
      ...
      if (!pass) reason = 'CSV requires consistent comma-separated columns across all lines';
      break;
    }
    case 'xml': {
      pass = /<[a-zA-Z][^>]*>[\s\S]*<\/[a-zA-Z][^>]*>/.test(received);
      if (!pass) reason = 'No XML tag pairs detected';
      break;
    }
    case 'yaml': {
      ...
      if (!pass) reason = 'Fewer than 50% of lines match YAML key: value pattern';
      break;
    }
    case 'table': {
      pass = /\|[^|]+\|/.test(received) && received.includes('|');
      if (!pass) reason = 'No table pipe (|) patterns detected';
      break;
    }
  }
...
export function toHaveListItems(received: string, items: string[]): MatcherResult {
